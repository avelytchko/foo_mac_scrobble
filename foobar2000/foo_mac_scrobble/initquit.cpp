...
static void log_api_credentials(const char* api_key, const char* api_secret, bool debug_enabled) {
    ...
    FB2K_console_formatter()
        << "Last.fm: Loaded API credentials - key length: " << key_len
    ...
    void on_init() override {
        ...
        if (g_session_manager->load_session(session_key, username)) {
            ...
            if (strlen(api_key) == 0 || strlen(api_secret) == 0) {
                FB2K_console_formatter()
                    << "Last.fm: Cannot validate session - missing API credentials";
                ...
                if (g_lastfm_api->validate_session()) {
                    ...
                    FB2K_console_formatter()
                        << "Last.fm Scrobbler: Authenticated as: " << username.c_str();
                } else {
                    FB2K_console_formatter()
                        << "Last.fm: Saved session invalid, clearing...";
                    g_lastfm_api->set_session_key("");
                    g_session_manager->clear_session();
                    cfg_session_key.set("");
                    cfg_username.set("");
            ...
            session_key_clean.erase(
                std::remove_if(session_key_clean.begin(), session_key_clean.end(),
            ...
            if (!session_key_clean.empty()) {
                ...
                if (g_lastfm_api->validate_session()) {
                    ...
                    FB2K_console_formatter()
                        << "Last.fm Scrobbler: Authenticated as: " << old_username.c_str();
                } else {
                    FB2K_console_formatter()
                        << "Last.fm: Config session invalid, clearing...";
                    g_lastfm_api->set_session_key("");
                    cfg_session_key.set("");
                    cfg_username.set("");
                }
        ...
        if (cfg_debug_enabled.get()) {
            ...
            FB2K_console_formatter()
                << "Last.fm: Scrobbling is "
                << (cfg_enabled.get() ? "ENABLED" : "DISABLED");
            FB2K_console_formatter()
                << "Last.fm: Scrobble threshold: "
                << cfg_scrobble_percent.get() << "%";
        }
