...
void log_debug(const char* fmt, ...) {
    if (!foo_lastfm::cfg_debug_enabled.get()) return;
    char buffer[1024];
    va_list args;
    va_start(args, fmt);
...
LastfmApi::LastfmApi() { log_debug("Last.fm: LastfmApi instance created"); }
LastfmApi::~LastfmApi() { log_debug("Last.fm: LastfmApi instance destroyed"); }
...
void LastfmApi::set_credentials(const char* api_key, const char* api_secret) {
    ...
    if (foo_lastfm::cfg_debug_enabled.get()) {
        FB2K_console_formatter()
            << "Last.fm: Credentials set (api_key: "
            << redact_secret(new_api_key).c_str()
            << " [len=" << new_api_key.length()
            << "], api_secret: "
            << redact_secret(new_api_secret).c_str()
...
void LastfmApi::set_session_key(const char* session_key) {
    ...
    if (new_session_key != m_session_key) {
        ...
        log_debug("Last.fm: Session key set (len=%d, value=%s)",
                  (int)m_session_key.length(),
...
bool LastfmApi::is_authenticated() const { return !m_session_key.empty(); }
...
bool LastfmApi::authenticate(const std::string& token) {
    ...
    std::string response;
    if (!send_api_request(params, response)) return false;
    ...
    try {
        ...
        FB2K_console_formatter()
            << "Last.fm JSON parse error: " << e.what();
        return false;
    }
}
...
bool LastfmApi::update_now_playing(const TrackInfo& track) {
    ...
    if (!track.album_artist.empty()) params["albumArtist"] = track.album_artist;
    if (track.duration > 0) params["duration"] = std::to_string(track.duration);
    if (track.track_number > 0) params["trackNumber"] = std::to_string(track.track_number);

    std::string response;
    bool ok = send_api_request(params, response);
    if (!ok) FB2K_console_formatter() << "Last.fm: Failed to update now playing";
    return ok;
}
...
bool LastfmApi::scrobble_track(const TrackInfo& track) {
    ...
    std::map<std::string, std::string> params{
        ...
        {"artist", track.artist},
        {"track", track.track},
        {"timestamp", std::to_string(track.timestamp)},
    };
    if (!track.album.empty()) params["album"] = track.album;
    if (!track.album_artist.empty()) params["albumArtist"] = track.album_artist;
    if (track.duration > 0) params["duration"] = std::to_string(track.duration);
    ...
    std::string response;
    bool ok = send_api_request(params, response);
    if (!ok) FB2K_console_formatter() << "Last.fm: Scrobble failed";
    return ok;
}
...
bool LastfmApi::validate_session() {
    ...
    if (curl_res == CURLE_COULDNT_CONNECT ||
        ...
        curl_res == CURLE_COULDNT_RESOLVE_HOST ||
        http_code == 0) {
        FB2K_console_formatter()
            << "Last.fm: Offline mode detected, skipping session validation";
        return true; // Keep session in offline mode
    }

    if (!ok || http_code == 403 || http_code == 401) {
        FB2K_console_formatter()
            << "Last.fm: Saved session is invalid (HTTP " << http_code << "), clearing...";
        m_session_key.clear();
        if (foo_lastfm::g_session_manager)
    ...
    FB2K_console_formatter()
        << "Last.fm: Session key validated (length: " << (int)m_session_key.length() << ")";
    return true;
}
...
bool LastfmApi::send_api_request(
    ...
    if (foo_lastfm::cfg_debug_enabled.get()) {
        if (method == "auth.getSession" || method == "auth.getToken") {
            ...
            FB2K_console_formatter()
                << "Post data length: " << (int)post_data.size();
        }
    }
    ...
    for (; attempt < 3; ++attempt) {
        ...
        if (http_code == 429 || (http_code >= 500 && http_code < 600)) {
            if (foo_lastfm::cfg_debug_enabled.get()) {
                FB2K_console_formatter()
                    << "Last.fm: Backing off for " << backoff_ms << " ms";
            }
    ...
    try {
        ...
        FB2K_console_formatter()
            << "Last.fm: Response not valid JSON (" << e.what() << ")";
        return false;
    }
...
void LastfmApi::execute_async_request(const std::map<std::string, std::string>& params, std::function<void(bool success, const std::string& response)> callback) {
    // Execute API request in a background thread
    std::thread([params, callback, this]() {
        std::string response;
        bool success = false;
        try {
        ...
        fb2k::inMainThread([callback, success, response]() {
...
void LastfmApi::authenticate_async(const std::string& token, std::function<void(bool success)> callback) {
    ...
    execute_async_request(params, [this, callback](bool success, const std::string& response) {
        if (success) {
            try {
                auto json_data = json::parse(response);
                if (json_data.contains("error")) {
...
void LastfmApi::update_now_playing_async(const TrackInfo& track) {
    ...
    execute_async_request(params, [](bool success, const std::string& response) {
